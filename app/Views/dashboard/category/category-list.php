...
?>
  <div id="content">
    <section>
      <card title="List">
        ...
        :options="{ 
          ...
          <template slot="action" scope="data">
            <div class="action">
              <hyperlink icon="edit"
              :to="baseURL(`dashboard/post/category/edit?id=${data.row.id}`)">
                Edit 
              </hyperlink>
      ...
      <floatarea>
        <button_ icon="plus" 
        :to="baseURL('dashboard/post/category/new')"></button_>
      </floatarea>
      ...
      methods: {
        deleteData(record) {
          confirm('Delete data?', () => {
            ...
            $.get(baseURL('dashboard/post/category/remove?id=' + record.id), (result) => {
              window.location = baseURL('dashboard/post/category');
            });
          });
        },
      },
